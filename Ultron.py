...
def help_menu():
   ...
   examples:
    ...
    ./Ultron.py -k --e my.email@email.com pwd.md --t target.email@email.com --v
    """)
...
def ultron():
    ...
    if len(sys.argv) == 1:
        ...
        if sys.argv[1] == "-help":
            ...
            if sys.argv[2] in ['-k', '-a', '-b', '-xss', '-e', '-h', '-d', '-g', '-i']:
                toolVersionDic = {
                '-k' : 'keylogger v.1.0.2',
                '-a' : 'arpspoofer v.1.0.1',
                '-b' : 'tool has no version yet',
                '-xss' : 'tool has no version yet',
                '-e' : 'email-tool v.1.0.1',
                '-g' : 'gen-tool v.1.0.2',
                '-d' : 'dos-tool v.1.0.2',
                '-i' : 'integrity-tool v.1.0.1',
                }
